...
namespace lab3
{
    class Program
    {
        ...
        static void FunctionOperations()
        {
            ...
            Console.WriteLine("1. F(x,y) = 5 + 3x - y");
            Console.WriteLine("2. G(x,y) = 7x - y");
        ...
        {
            int[] predefinedArray = { -1, 6, 1, 24, 1, 8, -21, 5, -5 };

            PrintHeader("Работа с предопределённым вектором");
            Console.WriteLine("Текущий вектор: [" + string.Join(", ", predefinedArray) + "]");
            Console.WriteLine();
            Console.WriteLine("Доступные операции:");
            Console.WriteLine("1. Сумма положительных элементов");
            Console.WriteLine("2. Произведение чётных элементов");
            ...
            int result = operation == 1
                ? Vector.SumPositiveElements(predefinedArray)
                : Vector.ProductOfEvenElements(predefinedArray);
        ...
        static void CustomVectorOperations()
        {
            ...
            vectorEx.OnCalculationRequested += (arr, condition) =>
            {
                try
                {
                    ...
                    int result = choice == 1
                        ? VectorEx.SumPositiveElements(arr)
                        : VectorEx.ProductOfEvenElements(arr);
